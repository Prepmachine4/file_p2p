...
class MyMainGui(QWidget, Ui_Form):
    ...
    def connect(self):
        put_text('FTP > 正在尝试进行连接', color="blue")
        try:
            from urlparse import urlparse
        except ImportError:
            from urllib.parse import urlparse

        result = str(self.hostEdit.text())
        host = str(result)
    ...
    def login(self):
        ask = ()
        if self.visitorRadio.isChecked():
            ask = ('Anonymous', '', True)
        else:
            ask = (str(self.nameEdit.text()), str(self.passwdEdit.text()), True)

        user, passwd = ask[:2]
        put_text(user + " " + passwd, "noshow")
        try:
            ftp_client.ftp_login(self.s, user, passwd)
            message = QMessageBox.information(self, '登陆成功', '已连接FTP服务器')
            self.initialize()
        except Exception as e:
